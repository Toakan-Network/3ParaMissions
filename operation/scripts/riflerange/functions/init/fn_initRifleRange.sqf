...
/*[_rangeID] spawn {
	...
	if (!isNil format ["%1_manual1", _rangeID]) then {
		...
		private _actionRead1 = [format ["%1_Read1",_rangeID],"Read","",{[_manualObj1,"textures\riflerange\Manual1.paa",RR_RANGE_MANUAL_TEXT] call bis_fnc_initInspectable},{true},{},[]] call ace_interact_menu_fnc_createAction;
	...
	if (!isNil format ["%1_manual2", _rangeID]) then {
		...
		private _actionRead2 = [format ["%1_Read2",_rangeID],"Read","",{[_manualObj2,"textures\riflerange\Manual2.paa",missionNamespace getVariable format ["%1_RANGE_MANUAL_TEXT",_rangeID]] call bis_fnc_initInspectable},{true},{},[]] call ace_interact_menu_fnc_createAction;
...
for "_l" from 1 to _laneCount do {
	...
	if !(isNil format ["%1_l%2_sign",_rangeID,_l]) then {
		missionNamespace getVariable format ["%1_l%2_sign",_rangeID,_l] setObjectTextureGlobal [0,format ["textures\riflerange\laneSign%1.paa",_l]];
	};
	...
	for "_g" from 1 to 4 do {
		...
		for "_n" from 0 to 99 do {
			...
			if (!isNil _targVarName) then {
				...
				_targ setObjectTextureGlobal [0,"textures\riflerange\figure11.paa"];

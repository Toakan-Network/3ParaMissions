...
if !(isPlayer _obj) then {
	_actionTestShort = _obj addAction ["Test Speaker: Short Sound", {[_this select 0, _this select 1, _this select 3] remoteExec ["RR_fnc_testSpeakerAction",2]}, [_rangeID,"sound\riflerange\BUZZER_ARENA_SHORT.wav"], 196, false, true, "", "", _radius];

	_actionTestLong = _obj addAction ["Test Speaker: Long Sound", {[_this select 0, _this select 1, _this select 3] remoteExec ["RR_fnc_testSpeakerAction",2]}, [_rangeID,"sound\riflerange\BUZZER_ARENA_LONG.wav"], 195, false, true, "", "", _radius];
};
